...
class ImageReconstructorSwitching:
            ...
            with CudaTimer('Reconstruction'):
                ...
                for channel in events_for_each_channel.keys():
                    with CudaTimer('Inference'):
                        ...
                        change_state = np.random.randint(2)
                        if change_state:
                            tic()
                            if self.mode == 0:
                                last_states = self.model.translate_Base2Reduced(self.last_states_for_each_channel[channel])
                                self.mode = 1
                            else:
                                last_states = self.model.translate_Reduced2Based(self.last_states_for_each_channel[channel])
                                self.mode = 0
                            print("Cost of context switch:", toc())
                        else:
                            last_states = self.last_states_for_each_channel[channel]
                        if self.mode == 0:
                            tic()
                            new_predicted_frame, states = self.model.forward_base(inputs,last_states)
                            print("MODE 0 cost: ", toc())
                        else:
                            tic()
                            new_predicted_frame, states = self.model.forward_reduced(inputs, last_states)
                            print("MODE 1 cost: ", toc())
